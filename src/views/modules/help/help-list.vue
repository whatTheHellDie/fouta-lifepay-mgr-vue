<template>
  <div class="mod-user">
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()"  label-width="40px">
      ...
      <el-form-item>
        <el-button type="primary"  v-if="isAuth('sys:help:save')" @click="addOrUpdateHandle()">{{$t( '添加' )}}</el-button>
        <el-button v-if="isAuth('sys:help:delete')" type="danger" @click="deleteHandle()" :disabled="dataListSelections.length <= 0">{{$t( '批量删除' )}}</el-button>
        <el-button type="primary"  v-if="isAuth('sys:help:save')" @click="addOrUpdateClassifyHandle()">{{$t( '添加分类' )}}</el-button>
      </el-form-item>
    </el-form>
    <el-table @selection-change="selectionChangeHandle" :data="dataList" border v-loading="dataListLoading"  style="width: 100%;">
      ...
      <el-table-column header-align="center" align="center" :label="$t( '分类' )"  prop="classIfyName"></el-table-column>
      ...
      <el-table-column fixed="right" header-align="center" align="center" :label="$t( '操作' )">
        <template slot-scope="scope">
          <el-button  type="text" v-if="isAuth('sys:help:update')" size="small" @click="addOrUpdateHandle(scope.row.id,scope.row.applicationName)">{{$t( '修改' )}}</el-button>
          <el-button  type="text" v-if="isAuth('sys:help:delete')" size="small" @click="deleteHandle(scope.row.id,scope.row.title)">{{$t( '删除' )}}</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import AddOrUpdate from './help-list-add-or-update'
  export default {
    data() {
      return {
    ...
    methods: {
       addOrUpdateClassifyHandle(id) {
        this.$router.push({path:'help-classify-list'})
      },
      // 新增 / 修改
      ...
      // 获取数据列表
       getDataList() {
        ...
        this.$http({
          url: this.$http.adornUrl('/sys/help/list'),
          method: 'post',
      ...
      deleteHandle (id, userName) {
        ...
        this.$confirm(this.$t( 'messages.template1',[userNames.join(','),id ? this.$t( '删除' ) : this.$t( '批量删除' )] ), this.$t( '提示' ), {
          ...
          this.$http({
            url: this.$http.adornUrl('/sys/help/delete'),
            method: 'post',
