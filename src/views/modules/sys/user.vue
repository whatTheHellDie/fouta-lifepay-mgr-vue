<template>
  <div class="mod-user">
    ...
    <el-table
      ...
      <el-table-column prop="email" header-align="center" align="center" :label="$t( '邮箱' )">
      </el-table-column>
      <el-table-column prop="mobile" header-align="center" align="center" :label="$t( '手机号' )">
      </el-table-column>
      <el-table-column prop="status" header-align="center" align="center" width="100" :label="$t( '状态' )">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.status === 0" size="small" type="danger">{{$t( '禁用' )}}</el-tag>
          <el-tag v-else size="small">{{$t( '正常' )}}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column fixed="right" header-align="center" align="center" width="180" :label="$t( '操作' )">
        <template slot-scope="scope">
          ...
          <el-button v-if="isAuth('sys:user:update')" type="text" size="small" @click="addOrUpdateHandle(scope.row.userId)">{{$t( '修改' )}}</el-button>
          <el-button v-if="isAuth('sys:user:delete')" type="text" size="small" @click="deleteHandle(scope.row.userId,scope.row.username)">{{$t( '删除' )}}</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <div class="phone-table">
      <div class="tr" v-for="item,i in dataList">
        ...
        <phone-column :prop="item.email" :label="$t( '邮箱' )"></phone-column>
        <phone-column :prop="item.mobile" :label="$t( '手机号' )"></phone-column>
        <phone-column :label="$t( '状态' )" :data="dataList" :index="i">
          <template slot-scope="scope">
          <el-tag v-if="scope.row.status === 0" size="small" type="danger">{{$t( '禁用' )}}</el-tag>
          <el-tag v-else size="small">{{$t( '正常' )}}</el-tag>
        </template>
