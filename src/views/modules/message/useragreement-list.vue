<template>
  <div class="mod-user">
    ...
    <el-table @selection-change="selectionChangeHandle" :data="dataList" border v-loading="dataListLoading" style="width: 100%;">
      ...
      <el-table-column header-align="center" align="center" :label="$t( '协议名称' )"  prop="types">
        <template slot-scope="scope">
          <div v-if="scope.row.types===0">{{$t('用户协议')}}</div>
          <div v-if="scope.row.types===1">{{$t('银行协议')}}</div>
          <div v-if="scope.row.types===2">{{$t('支付协议')}}</div>
        </template>
      </el-table-column>
      </el-table-column>
