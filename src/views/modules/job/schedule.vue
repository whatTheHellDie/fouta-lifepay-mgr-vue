    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button v-if="isAuth('sys:schedule:run')" type="text" size="small" @click="runHandle(scope.row.jobId,scope.row.beanName)">立即执行</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
  ...
  export default {
    ...
    methods: {
      ...
      runHandle (id,beanName) {
        ...
        this.$confirm(`确定对[${beanNames.join(',')}]进行[${id ? '立即执行' : '批量立即执行'}]操作?`, '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
